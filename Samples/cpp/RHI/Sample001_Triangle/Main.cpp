...
public:
    ...
    std::u8string_view title() const override { return u8"Sample001 - Triangle"; }
...
draco::Status TriangleSample::onInit() {
    using draco::Status;

    // Shader compiler.
    if (shaders::createCompiler(shaders::CompilerDesc{}, m_compiler) != draco::ErrorCode::Ok) return draco::ErrorCode::Unknown;

    if (sf::compileToModule(m_compiler, m_device, kShaderSource, shaders::ShaderStage::Vertex,   u8"VSMain", u8"TriangleVS", m_vs) != draco::ErrorCode::Ok) return draco::ErrorCode::Unknown;
    if (sf::compileToModule(m_compiler, m_device, kShaderSource, shaders::ShaderStage::Fragment, u8"PSMain", u8"TrianglePS", m_ps) != draco::ErrorCode::Ok) return draco::ErrorCode::Unknown;
